...
\begin{proposition}\label{prop:momentExist}
    ...
    \begin{enumerate}
        \item \texttt{Rv.} $X$의 $n$차 적률이 존재한다.
        \item 만약 $\expect(X)$가 존재한다면 $X$의 $n$차 중심화된 적률이 존재한다.
        \item 만약 $\var(X)$가 존재하고 $\var(X)\ne0$이면 $X$의 $n$차 표준화된 적률이 존재한다.
        \item \texttt{Rv.} $X$의 $n$차 하향계승적률이 존재한다.
        \item \texttt{Rv.} $X$의 $n$차 상향계승적률이 존재한다.
    \end{enumerate}
\end{proposition}

\begin{proof}
    i $\Leftrightarrow$ ii. 만약 $X$의 $n$차 적률이 존재하면 \texttt{Liapounov}의 부등식으로부터 모든 $i\leq n$에 대해 $X$의 $i$차 적률이 존재한다. 이로부터
    \begin{equation*}
        (X-\mu_X)^n=\sum_{i=0}^n(-1)^{n-i}\binom{n}{i}X^i\mu_X^{n-i}
    \end{equation*}
    에서 $X$의 $n$차 중심화된 적률이 존재한다. 역으로, $\expect(X)$가 존재하고 $X$의 $n$차 중심화된 적률이 존재하면 다시 \texttt{Liapounov}의 부등식으로부터 모든 $i\leq n$에 대해 $X$의 $i$차 중심화된 적률이 존재한다. 그렇다면
    \begin{align*}
        X^n&=(X-\mu_X+\mu_X)^n\\
        &=\sum_{i=0}^n\binom{n}{i}(X-\mu_X)^i\mu_X^{n-i}
    \end{align*}
    에서 $X$의 $n$차 적률이 존재한다.

    ii $\Leftrightarrow$ iii. 이는 정의로부터 자명하다.

    i $\Rightarrow$ iv. \texttt{Liapounov}의 부등식으로부터 모든 $i\leq n$에 대해 $X$의 $i$차 적률이 존재한다. 한편, 적당한 $a_0,\,\cdots,\,a_n\in\mathbb{R}$에 대해 $X^{\underline{n}}=\prod_{i=0}^{n-1}(X-i)=\sum_{i=0}^na_iX^i$이므로 $X$의 $n$차 하향계승적률이 존재한다. 이제 상향계승적률에 대해서도 비슷하게 하면 된다.

    iv $\Rightarrow$ i. 만약 $X$의 $n>1$차 계승하향적률이 존재한다면 $X^{\underline{n}}=X^{\underline{n-1}}(X-n+1)$에서
    \begin{align*}
        |X^{\underline{n-1}}|&=\bigg|\frac{X^{\underline{n}}}{X-n+1}\bigg|\ind_{\{X\ne n-1\}}+(n-1)!\ind_{\{X=n-1\}}\\
        &\leq|X^{\underline{n-1}}|\ind_{\{|X-n+1|<1\}}+|X^{\underline{n}}|\ind_{\{|X-n+1|\geq1\}}+(n-1)!\\
        &\leq n!+|X^{\underline{n}}|+(n-1)!
    \end{align*}
    이다. 여기서 마지막 부등호는 함수 $x\mapsto|x^{\underline{n-1}}|$이 $(n-2,\,\infty)$에서 증가하므로 성립한다. 이로부터 $X$의 $n-1$차 하향계승적률이 존재함을 알고, 이를 $n-2$번 반복하면 모든 $i\leq n$에 대해 $X$의 $i$차 하향계승적률이 존재함을 안다.

    이제 적당한 $a_0,\,\cdots,\,a_n\in\mathbb{R}$에 대해 $X^{\underline{n}}=\prod_{i=0}^{n-1}(X-i)=\sum_{i=0}^na_iX^i$이고 $a_n=1$임이 명백하므로 $X^n=X^{\underline{n}}-\sum_{i=0}^{n-1}a_iX^i$이다. 이를 $n-1$번 반복하면 다시 적당한 $b_0,\,\cdots,\,b_n\in\mathbb{R}$에 대해 $X^n=\sum_{i=0}^nb_iX^{\underline{i}}$이고, 앞서 보인 바와 같이 모든 $i\leq n$에 대해 $X$의 $i$차 하향계승적률이 존재하므로 $X$의 $n$차 적률도 존재한다.

    i $\Leftrightarrow$ v. 이는 i $\Leftrightarrow$ iv의 증명과 비슷하게 하면 된다.
\end{proof}

\begin{proposition}
    확률공간 $(\Omega,\,\mathcal{F},\,\mathbb{P})$에서 정의된 \texttt{rv.} $X:\Omega\to\mathbb{R}$에 대해 만약 $X$의 $m$차 적률(혹은 중심화된 적률, 표준화된 적률, 하향계승적률, 상향계승적률)이 존재하면 $X$의 $n\leq m$차 적률(혹은 중심화된 적률, 표준화된 적률, 하향계승적률, 상향계승적률)이 존재한다.
\end{proposition}
